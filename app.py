...
# 2. ตั้งค่าคำถาม-คำตอบ (ดักจับคำสำคัญจากประโยคที่ผู้ใช้พิมพ์)
FAQ_RESPONSES = {
    ...
    "ครูแจ้": "สวัสดีครับ ผมแจ้ คนหล่อหน้าตาดี ตัวตึงสายคอนเท้น" # (Key ลับ บังคับตอบตามนี้เป๊ะๆ)
}

# 3. สร้างระบบจดจำประวัติการแชต (เพื่อให้หน้าจอแสดงแชตค้างไว้ ไม่หายไปตอนพิมพ์ใหม่)
...
def simulate_streaming(text):
    for char in text:
        yield char
        time.sleep(0.02) # ความเร็วในการพิมพ์
...
if prompt := st.chat_input("พิมพ์คำถามของคุณที่นี่... (ลองพิมพ์คำว่า 'ครูแจ้')"):
    ...
    # 6. ค้นหาคำตอบจากคำสำคัญ (Keyword Matching)
    # คำตอบเริ่มต้น (กรณีพิมพ์มาแล้วไม่ตรงกับ 6 ข้อด้านบน)
    bot_reply = "ขออภัยครับ บอตไม่เข้าใจในคำถามนะครับ รบกวนพิมพ์คำถามใหม่ หรือทิ้งเบอร์โทรไว้ให้ช่างติดต่อกลับนะครับ 🙏"
    
    for keyword, response in FAQ_RESPONSES.items():
        # ถ้าคำสำคัญ (เช่น "เปิด", "คิว") อยู่ในประโยคที่ผู้ใช้พิมพ์มา ให้ดึงคำตอบนั้นไปใช้
    ...
    # บันทึกคำตอบของ AI ลงในประวัติแชต
